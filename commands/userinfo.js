module.exports = {
  ...
  aliases: ['ui'],
	...
	async execute(client,message,args) {
        ...
        let user = message.mentions.users.first() || message.guild.members.cache.find(member => member.user.id === args.join(' ')) || message.guild.members.cache.find(member => member.user.nickname.toLowerCase() === args.join(' ').toLowerCase() || member.user.username.toLowerCase() === args.join(' ').toLowerCase()) || message.author;
        let isabot = ''
